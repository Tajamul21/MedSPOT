...
# Aguvis system prompt
AGUVIS_SYSTEM_PROMPT = (
...
def load_model(model_path):
    print(f"Loading AGUVIS model: {model_path}")
...
def smart_resize_dims(W, H):
    """Get the smart-resized dimensions that Qwen2-VL processor uses."""
...
def extract_point_from_text(text, W, H):
    ...
    patterns = [
        r"pyautogui\.click\s*\(\s*x\s*=\s*([\d.]+)\s*,\s*y\s*=\s*([\d.]+)\s*\)",
    ...
    for p in patterns:
        ...
        if m:
            ...
            if px > 1.0 or py > 1.0:
                resized_W, resized_H = smart_resize_dims(W, H)
                px = px / resized_W
                py = py / resized_H
...
def evaluate_step(model, processor, image_path, instruction, gt_bbox):
    ...
    messages = [
        {
            "role": "system",
            "content": [{"type": "text", "text": AGUVIS_SYSTEM_PROMPT}],
        },
        {
            "role": "user",
            "content": [
...
"""
Run:
    python evaluate_Aguvis.py \
        --model_name_or_path xlangai/Aguvis-7B-720P \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name_or_path", default="xlangai/Aguvis-7B-720P")
    ...
    RESULTS_ROOT = REPO_ROOT / "Results" / "AGUVIS-7B"
