...
Run:
    python evaluate_deepseek.py \
        --model_name_or_path deepseek-ai/deepseek-vl2 \
...
def extract_point_from_text(text, W, H):
    ...
    for p in patterns:
        ...
        if m:
            ...
            # DeepSeek outputs normalized 0-1 directly
            # Only divide if clearly pixel values (> 1)
            if px > 1.0 or py > 1.0:
                px, py = px / W, py / H
...
def evaluate_step(model, processor, image_path, instruction, gt_bbox):
    ...
    prompt = (
        f"{instruction}\n\n"
        "Look carefully at the screenshot and locate the UI element.\n"
        "Return ONLY the normalized coordinates (x,y) where x and y are between 0 and 1.\n"
        "Format strictly: (x,y)\n"
        "Do NOT explain."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name_or_path", default="deepseek-ai/deepseek-vl2")
    ...
    RESULTS_ROOT = REPO_ROOT / "Results" / "DeepSeekVL2"
