...
def evaluate_step(model, tokenizer, image_path, instruction, gt_bbox):
    ...
    returns : (hit: bool, reason: str, pred_point: tuple|None)
    """
    ...
    prompt = (
        ...
        "Look carefully at the image and identify the exact UI element described.\n"
        "Return the pixel coordinates of the CENTER of that UI element.\n"
        "Format strictly: (x,y)\n"
        "Do NOT return the image center. Do NOT explain."
    )
    ...
    if tokenized.get("image_sizes") is None and tokenized.get("pixel_values") is not None:
        pv = tokenized["pixel_values"]   # [1, 3, pH, pW]
        tokenized["image_sizes"] = torch.tensor(
            [[pv.shape[2], pv.shape[3]]], dtype=torch.long, device=device
        )
    ...
    if pred_point is not None:
        ...
        if do_boxes_overlap(patch, list(gt_bbox)):
            return True, "hit_overlap", pred_point

    return False, classify_failure(pred_point, gt_bbox), pred_point
...
def evaluate_task(task, model, tokenizer, dataset_root):
    ...
    for idx, step in enumerate(steps, start=1):
        ...
        hit, reason, pred_point = evaluate_step(
...
Run:
    python evaluate_mistral.py \
        --model_name_or_path mistralai/Mistral-Small-3.1-24B-Instruct-2503 \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name_or_path", default="mistralai/Mistral-Small-3.1-24B-Instruct-2503")
    ...
    RESULTS_ROOT = REPO_ROOT / "Results" / "Mistral"
